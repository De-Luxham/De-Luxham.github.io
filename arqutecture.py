...
# def choose_qubits(n=5,arq=ibmq_16_melbourne):
    ...
    # templist = []
    # for item in arq["connections"]:
        # if item["source"] == start or item["target"] == start:
            # if item["source"] == start:
                # qubits.append(item["target"])
            # if item["target"] == start:
                # qubits.append(item["source"])
            # #templist.append(item)
...
# run these together arq(qubits=choose_qubits(n=5))
